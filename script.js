...
const game = {
    // used for calculating win rate and displaying game round
    roundCounter: 0,
    /**
    ...
    computeScore(winner, loser, fan, isSelfDraw = false) {
        isSelfDraw ? this.selfDraw(winner, fan) : this.fromDiscard(winner, loser, fan);
    },
};
...
const winDeclaration = (() => {
    ...
    winForm.addEventListener("submit", (e) => {
        ...
        if (self && winner !== "draw") game.computeScore(winner, undefined, fan, true);
        if (discard) {
